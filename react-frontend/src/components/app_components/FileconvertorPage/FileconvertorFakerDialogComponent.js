...
import fileconvertorFakerFactory from "./fileconvertorFakerFactory";
...
const FileconvertorFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = fileconvertorFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("fileconvertor").create(elem));
...
export default connect(mapState, mapDispatch)(FileconvertorFakerDialogComponent);

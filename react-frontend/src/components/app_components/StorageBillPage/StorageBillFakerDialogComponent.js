...
import storageBillFakerFactory from "./storageBillFakerFactory";
...
const StorageBillFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = storageBillFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("storageBill").create(elem));
...
export default connect(mapState, mapDispatch)(StorageBillFakerDialogComponent);

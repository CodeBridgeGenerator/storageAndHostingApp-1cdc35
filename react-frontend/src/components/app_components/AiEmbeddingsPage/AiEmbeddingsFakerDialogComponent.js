...
import aiEmbeddingsFakerFactory from "./aiEmbeddingsFakerFactory";
...
const AiEmbeddingsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = aiEmbeddingsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("aiEmbeddings").create(elem));
...
export default connect(mapState, mapDispatch)(AiEmbeddingsFakerDialogComponent);

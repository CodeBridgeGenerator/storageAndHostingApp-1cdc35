...
import hostingFakerFactory from "./hostingFakerFactory";
...
const HostingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = hostingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("hosting").create(elem));
...
export default connect(mapState, mapDispatch)(HostingFakerDialogComponent);

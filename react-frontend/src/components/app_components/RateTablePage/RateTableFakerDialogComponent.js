...
import rateTableFakerFactory from "./rateTableFakerFactory";
...
const RateTableFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = rateTableFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("rateTable").create(elem));
...
export default connect(mapState, mapDispatch)(RateTableFakerDialogComponent);

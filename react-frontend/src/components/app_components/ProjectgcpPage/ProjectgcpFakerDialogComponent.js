...
import projectgcpFakerFactory from "./projectgcpFakerFactory";
...
const ProjectgcpFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = projectgcpFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("projectgcp").create(elem));
...
export default connect(mapState, mapDispatch)(ProjectgcpFakerDialogComponent);

...
const EmailDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.emailMessages}</p>
...
<Column field="emailMessages" header="Email Messages" body={pTemplate1} filter={selectedFilterFields.includes("emailMessages")} hidden={selectedHideFields?.includes("emailMessages")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Email Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="email"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Email" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"

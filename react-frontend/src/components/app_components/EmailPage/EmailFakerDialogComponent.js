...
import emailFakerFactory from "./emailFakerFactory";
...
const EmailFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = emailFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("email").create(elem));
...
export default connect(mapState, mapDispatch)(EmailFakerDialogComponent);

  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      toast.current.show({
        severity: "success",
        summary: "Copied",
        detail: `profiles data copied to clipboard`,
        life: 3000,
      });
      deselectAllRows();
      setRefresh((prev) => !prev);
  ...
  const handleDuplicate = async () => {
    ...
    try {
      const promises = selectedItems.map((item) => {
        ...
        return client.service("profiles").create(newItem);
      });
      const createdItems = await Promise.all(promises);
      toast.current.show({
        severity: "success",
        summary: "Duplicated",
        detail: `${selectedItems.length} profiles duplicated successfully`,
        life: 3000,
      });
      deselectAllRows();
      setRefresh((prev) => !prev);
    } catch (error) {
      console.error("Failed to duplicate profiles", error);
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "Failed to duplicate profiles",
        life: 3000,
      });
    }
  };

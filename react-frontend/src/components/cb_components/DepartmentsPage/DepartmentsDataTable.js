  ...
  const confirmDelete = async () => {
    try {
      const promises = selectedItems.map((item) =>
        client.service("departments").remove(item._id),
      );
    ...
    const fetchPermissions = async () => {
      ...
      try {
        if (selectedUser) {
          ...
          const departmentsPermissions = await client
            .service("permissionServices")
            .find({
              query: { service: "departments" },
            });

          let userPermissions = null;

          // Priority 1: Profile
          userPermissions = departmentsPermissions.data.find(
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      toast.current.show({
        severity: "success",
        summary: "Copied",
        detail: `departments data copied to clipboard`,
        life: 3000,
      });
      deselectAllRows();
      setRefresh((prev) => !prev);
  ...
  const handleDuplicate = async () => {
    ...
    try {
      const promises = selectedItems.map((item) => {
        ...
        return client.service("departments").create(newItem);
      });
      const createdItems = await Promise.all(promises);
      toast.current.show({
        severity: "success",
        summary: "Duplicated",
        detail: `${selectedItems.length} departments duplicated successfully`,
        life: 3000,
      });
      deselectAllRows();
      setRefresh((prev) => !prev);
    } catch (error) {
      console.error("Failed to duplicate departments", error);
      toast.current.show({
        severity: "error",
        summary: "Error",
        detail: "Failed to duplicate departments",
        life: 3000,
      });
    }
  };

...
const AuditsDataTable = ({
  ...
  const serviceNameTemplate = (rowData) => {
    return <p>{rowData.serviceName}</p>; // Assuming serviceName is a string
  };

  const actionTemplate = (rowData) => {
    return <p>{rowData.action}</p>; // Assuming action is a string
  };
  const methodTemplate = (rowData) => {
    return <p>{rowData.method}</p>; // Assuming method is a string
  };
  const detailsTemplate = (rowData) => {
    return <p>{rowData.details}</p>; // Assuming details is a string
  };

  const createdByTemplate = (rowData) => {
    // Check if createdBy exists and has a name property before rendering
  ...
  const pTemplate2 = (rowData, { rowIndex }) => <p>{rowData.method}</p>;
  const pTemplate3 = (rowData, { rowIndex }) => <p>{rowData.details}</p>;
  const pTemplate4 = (rowData, { rowIndex }) => <p>{rowData.createdBy}</p>;
    ...
    <>
      ...
      >
        ...
        <Column
          ...
          header="Method"
          body={methodTemplate}
          filter={selectedFilterFields.includes("method")}
          hidden={selectedHideFields?.includes("method")}
          sortable
          style={{ minWidth: "8rem" }}
        />
        <Column
